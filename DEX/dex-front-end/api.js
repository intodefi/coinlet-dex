...
API.init = function init(callback, allContracts, path, provider, configName, lookbackIn) {
  ...
  this.web3.version.getNetwork((error, version) => {
    ...
    // path
    if (path) {
      this.config.contractBitDex = path + this.config.contractBitDex;
      this.config.contractToken = path + this.config.contractToken;
    }

    // contracts
    this.contractBitDex = undefined;
    this.contractBitDexAddrs = [this.config.contractBitDexAddrs[0].addr];
    if (allContracts) {
      this.contractBitDexAddrs = this.config.contractBitDexAddrs.map(x => x.addr);
    }
    ...
    async.series(
      [
        (callbackSeries) => {
          this.utility.loadContract(
            ...
            this.contractBitDexAddrs[0],
            (err, contract) => {
              this.contractBitDex = contract;
      ...
      () => {
        callback(null, {
          contractBitDex: this.contractBitDex,
...
API.logs = function logs(callback, lookbackIn) {
  this.utility.blockNumber(this.web3, (err, blockNumber) => {
    ...
    async.mapSeries(
      this.contractBitDexAddrs,
      (contractBitDexAddr, callbackMap) => {
        const blocks = Object.values(this.eventsCache)
          .filter(x => x.address === contractBitDexAddr)
          .map(x => x.blockNumber);
...
API.getBitDexBalance = function getBitDexBalance(addr, callback) {
  if (addr.length === 42) {
...
API.getBitDexTokenBalances = function getBitDexTokenBalances(addr, callback) {
  if (addr.length === 42) {
    async.reduce(
      this.config.tokens,
      {},
...
API.getUSDBalance = function getUSDBalance(addr, tokenPrices, callback) {
  async.parallel(
    [
      ...
      (callbackParallel) => {
        API.getBitDexTokenBalances(addr, callbackParallel);
      },
      (callbackParallel) => {
    ...
    (err, results) => {
      const balances = { Wallet: results[0], BitDex: results[1] };
...
API.addOrderFromEvent = function addOrderFromEvent(event, callback) {
  if (event.event === 'Order' && event.address === this.contractBitDexAddrs[0]) {
...
API.getTrades = function getTrades(callback) {
  ...
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractBitDexAddrs.indexOf(event.address) >= 0) {
...
API.getDepositsWithdrawals = function getDepositsWithdrawals(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Deposit' && this.contractBitDexAddrs.indexOf(event.address >= 0)) {
      ...
      event.event === 'Withdraw' && this.contractBitDexAddrs.indexOf(event.address) >= 0
    ) {
      if (event.args.amount.toNumber() > 0) {
        const token = API.getToken(event.args.token);
        depositsWithdrawals.push({
      ...
      if (trade.token && trade.base && trade.base.name === 'ETH') {
        ...
        if (!tickers[pair]) {
          ...
          ordersFiltered = ordersFiltered.filter(
            order => order.order.contractAddr === this.config.contractBitDexAddrs[0].addr);
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        this.utility.sign(this.web3, addr, hash, pk, (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.contractBitDexAddrs[0],
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
