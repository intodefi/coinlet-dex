...
BitDex.prototype.dialogInfo = function dialogInfo(message) {
  console.log(message);
...
BitDex.prototype.dialogError = function dialogError(message) {
  console.log(message);
...
BitDex.prototype.alertSuccess = function alertSuccess(message) {
  console.log(message);
...
BitDex.prototype.txError = function txError(err) {
  ...
  utility.getBalance(this.web3, this.addrs[this.selectedAccount], (errBalance, resultBalance) => {
    ...
    if (this.connection.connection === 'RPC' && this.addrKinds[this.selectedAccount] !== 'MetaMask') {
      if (this.pks[this.selectedAccount]) {
        this.dialogError('You are using an BitDex account that has a private key attached, but you\'re connected to MetaMask. You should disable MetaMask from Chrome\'s Window -> Extensions menu (don\'t worry, this won\'t lose your MetaMask data), then refresh BitDex.');
        ga('send', {
          hitType: 'event',
          eventCategory: 'Error',
      ...
      this.dialogError('You are using an BitDex account that doesn\'t have a private key attached. Perhaps you created the account using MetaMask, in which case you should make sure MetaMask is enabled and logged in to this account, then refresh BitDex. Or, if you have the private key, you can choose "Import account" from the accounts dropdown (upper right) to re-import the account with its private key.');
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
      ...
      this.dialogError('You are using an BitDex account that has an invalid private key.');
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
      ...
      this.dialogError(
        `Your wallet's ETH balance (${balance} ETH) is not enough to cover the gas cost (Ethereum network fee). BitDex sends ${this.minGas} ETH with each transaction. This is an overestimate and the excess will get refunded to you. It's a good idea to send more than ${this.minGas} so you can pay for not only this transaction, but also future transactions you do on BitDex. The gas has to come directly from your Wallet (BitDex has no physical way of paying gas from your deposited ETH).`);
      ga('send', {
        hitType: 'event',
        eventCategory: 'Error',
        eventAction: 'Ethereum - transaction error',
...
BitDex.prototype.alertTxResult = function alertTxResult(err, txsIn) {
...
BitDex.prototype.enableTooltipsAndPopovers = function enableTooltipsAndPopovers() {
  $('[data-toggle="popover"]').popover({ trigger: 'hover' });
  $('[data-toggle="tooltip"]').tooltip();
};
BitDex.prototype.createAccount = function createAccount() {
...
BitDex.prototype.deleteAccount = function deleteAccount() {
  if (this.pks[this.selectedAccount]) {
...
BitDex.prototype.selectAccount = function selectAccount(i) {
...
BitDex.prototype.addAccount = function addAccount(newAddr, newPk) {
...
BitDex.prototype.showPrivateKey = function showPrivateKey() {
  const addr = this.addrs[this.selectedAccount];
  const pk = this.pks[this.selectedAccount];
  if (!pk) {
    this.dialogError(
...
BitDex.prototype.addressLink = function addressLink(address) {
  return `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${address}`;
};
BitDex.prototype.contractAddr = function contractAddr(addr) {
  this.config.contractBitDexAddr = addr;
  this.displayConnectionDescription(() => {});
  this.loading(() => {});
  this.refresh(() => {}, true, true);
};
BitDex.prototype.displayAccounts = function displayAccounts(callback) {
...
BitDex.prototype.displayLanguages = function displayLanguages(callback) {
...
BitDex.prototype.selectLanguage = function selectLanguage(newLanguage) {
...
BitDex.prototype.loadEvents = function loadEvents(callback) {
  let lastBlock = 0;
  Object.keys(this.eventsCache).forEach((id) => {
    const event = this.eventsCache[id];
    if (event.blockNumber > lastBlock && event.address === this.config.contractBitDexAddr) {
...
BitDex.prototype.displayMyTransactions =
function displayMyTransactions(ordersIn, blockNumber, callback) {
  ...
  orders = orders.filter(order => order.order.contractAddr === this.config.contractBitDexAddr);
  ...
  events.forEach((event) => {
    try {
      if (
        event.event === 'Trade' &&
        event.address === this.config.contractBitDexAddr &&
...
BitDex.prototype.isInCross = function isInCross(priceIn, kind) {
  ...
  .filter(
    order => order.order.contractAddr === this.config.contractBitDexAddr);
  // final order filtering and sorting
...
BitDex.prototype.displayVolumes = function displayVolumes(
...
BitDex.prototype.displayTradesAndChart = function displayTradesAndChart(callback) {
  // get the trade list
  const events = Object.values(this.eventsCache);
  const trades = [];
  events.forEach((event) => {
    if (event.event === 'Trade' && event.address.toLowerCase() === this.config.contractBitDexAddr.toLowerCase()) {
      if (event.args.amountGive.toNumber() > 0 && event.args.amountGet.toNumber() > 0) {
        // don't show trades involving 0 amounts
        let trade;
        if (
...
BitDex.prototype.candlestickChart =
...
BitDex.prototype.getOrdersByPair = function getOrdersByPair(tokenA, tokenB, callback) {
...
BitDex.prototype.getReturnTicker = function getTopOrders(callback) {
...
BitDex.prototype.getTopOrders = function getTopOrders(callback) {
...
BitDex.prototype.displayOrderbook = function displayOrderbook(ordersIn, blockNumber, callback) {
  // only look at orders for the selected token and base
  let orders = ordersIn.filter(
    x =>
      (x.order.tokenGet.toLowerCase() === this.selectedToken.addr.toLowerCase() &&
...
BitDex.prototype.displayTokensAndBases = function displayTokensAndBases(callback) {
...
BitDex.prototype.displayAllBalances = function displayAllBalances(callback) {
...
BitDex.prototype.transfer = function transfer(addr, inputAmount, toAddr) {
  ...
  } else if (toAddr.toLowerCase() === this.config.contractBitDexAddr.toLowerCase()) {
    this.dialogError('If you send funds directly to the BitDex smart contract, they will be lost. You need to use the Deposit tab to deposit.');
    ga('send', {
      hitType: 'event',
      eventCategory: 'Error',
      eventAction: 'Transfer - invalid address',
...
BitDex.prototype.deposit = function deposit(addr, inputAmount) {
  let amount = new BigNumber(Number(utility.ethToWei(inputAmount, this.getDivisor(addr))));
  const token = this.getToken(addr);
  if (amount.lte(0)) {
  ...
  if (addr.slice(0, 39) === '0x0000000000000000000000000000000000000') {
    ...
    utility.call(
      ...
      token.addr,
      'allowance',
      [this.addrs[this.selectedAccount], this.config.contractBitDexAddr],
                  ...
                  (callbackSeries) => {
                    if (resultAllowance.eq(0)) {
                      utility.send(
                        ...
                        addr,
                        'approve',
                        [this.config.contractBitDexAddr, amount,
...
BitDex.prototype.withdraw = function withdraw(addr, amountIn) {
...
BitDex.prototype.order = function order(direction, amount, price, expires, refresh) {
...
BitDex.prototype.publishOrder = function publishOrder(
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        hash, this.pks[this.selectedAccount], (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.config.contractBitDexAddr,
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
...
BitDex.prototype.cancelOrder = function cancelOrder(orderIn) {
...
BitDex.prototype.trade = function trade(kind, order, inputAmount) {
  if (this.addrs[this.selectedAccount].slice(0, 39) === '0x0000000000000000000000000000000000000') {
    this.dialogError(
      "You haven't selected an account. Make sure you have an account selected from the Accounts dropdown in the upper right.");
    ga('send', {
...
BitDex.prototype.addPending = function addPending(err, txsIn) {
  const txs = Array.isArray(txsIn) ? txsIn : [txsIn];
  txs.forEach((tx) => {
...
BitDex.prototype.updateUrl = function updateUrl() {
...
BitDex.prototype.getDivisor = function getDivisor(tokenOrAddress) {
...
BitDex.prototype.setGasPrice = function setGasPrice(gasPrice) {
  if (gasPrice) {
...
BitDex.prototype.displayBuySell = function displayBuySell(callback) {
...
BitDex.prototype.displayTokenGuidesDropdown = function displayTokenGuidesDropdown() {
  const tokens = this.config.tokens.map(x => x);
  tokens.sort((a, b) => (a.name > b.name ? 1 : -1));
...
BitDex.prototype.displayHelpDropdown = function displayHelpDropdown() {
  this.ejs(`${this.config.homeURL}/templates/helpDropdown.ejs`, 'helpDropdown', {});
};
BitDex.prototype.displayHelp = function displayHelp(name) {
  $('#helpBody').html('');
...
BitDex.prototype.displayScreencast = function displayScreencast(name) {
  $('#screencastBody').html('');
...
BitDex.prototype.displayConnectionDescription = function displayConnectionDescription() {
  this.ejs(`${this.config.homeURL}/templates/connectionDescription.ejs`, 'connection', {
    connection: this.connection,
    contracts: this.config.contractBitDexAddrs,
    contractAddr: this.config.contractBitDexAddr,
    contractLink: `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${this.config.contractBitDexAddr}`,
  });
};
BitDex.prototype.displayTokenGuide = function displayTokenGuide(name) {
...
BitDex.prototype.checkContractUpgrade = function checkContractUpgrade() {
  if (
    (!this.selectedContract || this.selectedContract !== this.config.contractBitDexAddr) &&
    (this.addrs.length > 1 ||
      (this.addrs.length === 1 && this.addrs[0].slice(0, 39) !== '0x0000000000000000000000000000000000000'))
  ) {
    this.dialogInfo(
      '<p>BitDex has a new smart contract. It is now selected.</p><p>Please use the "Smart Contract" menu to select the old one and withdraw from it.</p><p><a href="javascript:;" class="btn btn-default" onclick="alertify.closeAll(); bundle.BitDex.displayHelp(\'smartContract\')">Smart contract changelog</a></p>');
  }
};
BitDex.prototype.resetCaches = function resetCaches() {
...
BitDex.prototype.loading = function loading(callback) {
  [
    'deposit',
    'withdraw',
    'buy',
...
BitDex.prototype.refresh = function refresh(callback, forceEventRead, initMarket, token, base) {
  this.q.push((done) => {
    if (token && base) {
    ...
    this.selectedContract = this.config.contractBitDexAddr;
...
BitDex.prototype.refreshLoop = function refreshLoop() {
  const self = this;
  function loop() {
    self.refresh(() => {
...
BitDex.prototype.initDisplays = function initDisplays(callback) {
  this.loading(() => {});
...
BitDex.prototype.loadWeb3 = function loadWeb3(callback) {
  this.config = config;
  // web3
...
BitDex.prototype.initContracts = function initContracts(callback) {
  this.web3.version.getNetwork((error, version) => {
    ...
    // connection
    this.config.contractBitDexAddr = this.config.contractBitDexAddrs[0].addr;
    // get accounts
...
BitDex.prototype.startBitDex = function startBitDex() {
...
const bitDex = new BitDex();

module.exports = { BitDex: bitDex, utility };
