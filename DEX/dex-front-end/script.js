...
$(() => {
  $('body').on('click', '#accountSubmit', (e) => {
    ...
    bundle.BitDex.addAccount($('#accountAddr').val(), $('#accountPk').val());
  });
});
function buyChange() { // eslint-disable-line no-unused-vars
  ...
  const isInCross = bundle.BitDex.isInCross(price, 'buy');
  if (isInCross) {
...
function sellChange() { // eslint-disable-line no-unused-vars
  ...
  const total = (amount * price).toFixed(3);
  const isInCross = bundle.BitDex.isInCross(price, 'sell');
  if (isInCross) {
...
$(() => {
  $('body').on('click', '#buyCrossSubmit', (e) => {
    ...
    bundle.BitDex.trade('buy', JSON.parse($('#buyCrossOrder').val()), $('#buyCrossAmount').val());
  });
});
$(() => {
...
$('#gasPriceModal').on('show.bs.modal', () => {
  $('#gasPrice').val(bundle.BitDex.config.ethGasPrice / 1000000000);
});
$(() => {
  $('body').on('click', '#gasPriceSubmit', (e) => {
    e.preventDefault();
    $('#gasPriceModal').modal('hide');
    bundle.BitDex.setGasPrice(
      $('#gasPrice').val());
  });
});
function depositClick(addr) { // eslint-disable-line no-unused-vars
  bundle.BitDex.deposit(addr, $(`#depositAmount${addr}`).val());
}
function withdrawClick(addr) { // eslint-disable-line no-unused-vars
  bundle.BitDex.withdraw(addr, $(`#withdrawAmount${addr}`).val());
}
function transferClick(addr) { // eslint-disable-line no-unused-vars
  bundle.BitDex.transfer(addr, $(`#transferAmount${addr}`).val(), $(`#transferTo${addr}`).val());
}
$(() => {});
